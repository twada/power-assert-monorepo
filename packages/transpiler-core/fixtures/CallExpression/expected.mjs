...
describe('description', () => {
  it('function', () => {
    ...
    const func = () => false;
    _pasrt1.run(_parg1.rec(func(), 7, 7, 13));
  });
  it('method callee is non-computed MemberExpression', () => {
    ...
    _pasrt2.run(_parg2.rec(_parg2.tap(obj, 7, 7, 10).method(), 11, 7, 19));
  });
  it('method callee is computed MemberExpression', () => {
    ...
    _pasrt3.run(_parg3.rec(_parg3.tap(obj, 7, 7, 10)[_parg3.tap(methodName, 11, 11, 21)](), 22, 7, 24));
  });
  it('method callee is function', () => {
    ...
    _pasrt4.run(_parg4.rec(_parg4.tap(inner(), 7, 7, 14).exact(), 15, 7, 22));
  });
  it('CallExpression of CallExpression of CallExpression', () => {
    ...
    const outer = () => () => () => false;
    _pasrt5.run(_parg5.rec(_parg5.tap(_parg5.tap(outer(), 7, 7, 14)(), 14, 7, 16)(), 16, 7, 18));
  });
  it('method callee is non-computed MemberExpression that returns function then invoke immediately', () => {
    ...
    _pasrt6.run(_parg6.rec(_parg6.tap(_parg6.tap(_parg6.tap(obj, 7, 7, 10).method(), 11, 7, 19)(), 19, 7, 21)(), 21, 7, 23));
  });
});
