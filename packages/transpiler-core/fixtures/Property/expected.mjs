...
import {_power_} from "@power-assert/runtime";
{
  ...
  _pasrt1.run(_parg1.rec({
    [_parg1.tap(num, 9, 9, 12, 1)]: _parg1.tap(foo, 15, 15, 18, 2)
  }, 7, 7, 19, 0), _parg2.rec('Computed (dynamic) property names', 21, 21, 56, 0));
}
{
  const _pasrt2 = _power_(assert, null, "assert({[ 'prop_' + foo() ]: 42})");
  const _parg3 = _pasrt2.recorder(0);
  _pasrt2.run(_parg3.rec({
    [_parg3.tap(_parg3.tap('prop_', 10, 10, 17, 2) + _parg3.tap(foo(), 20, 20, 25, 3), 18, 10, 25, 1)]: _parg3.tap(42, 29, 29, 31, 4)
  }, 7, 7, 32, 0));
}
{
  ...
  _pasrt3.run(_parg4.rec({
    [_parg4.tap(`prop_${_parg4.tap(generate(_parg4.tap(seed, 26, 26, 30, 3)), 17, 17, 31, 2)}`, 9, 9, 33, 1)]: _parg4.tap(foo, 36, 36, 39, 4)
  }, 7, 7, 40, 0));
}
{
  ...
  _pasrt4.run(_parg5.rec({
    foo
  }, 7, 7, 12, 0), _parg6.rec('shorthand literal itself will not be instrumented', 14, 14, 65, 0));
}
{
