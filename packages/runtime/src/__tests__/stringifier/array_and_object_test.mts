...
describe('stringify Array with indentation', () => {
  ...
  it('3 items array', () => {
    ...
    const expected = ['[', '  3,', '  5,', '  8', ']'].join('\n');
    assert.equal(stringify(input, { indent: '  ' }), expected);
  });
  it('nested array', () => {
    const input = [4, [5, [6, 7, 8], 9], 10];

    const expected = ['[', '  4,', '  [', '    5,', '    [', '      6,', '      7,', '      8', '    ],', '    9', '  ],', '  10', ']'].join('\n');
    assert.equal(stringify(input, { indent: '  ' }), expected);
  });
  it('nested empty array', () => {
    const input = [3, [], 8];

    const expected = ['[', '  3,', '  [],', '  8', ']'].join('\n');
    assert.equal(stringify(input, { indent: '  ' }), expected);
  });
  it('nested array with maxDepth option', () => {
    const input = [3, [4, 5], 8];

    const expected = ['[', '  3,', '  #Array#,', '  8', ']'].join('\n');
...
describe('stringify Object with indentation', () => {
  ...
  it('two props object', () => {
    ...
    const expected = ['Object{', '  name: "bob",', '  age: 3', '}'].join('\n');
    assert.equal(stringify(input, { indent: '  ' }), expected);
  });
  it('nested object', () => {
    const input = { a: 'A', b: { ba: 'BA', bb: 'BB' }, c: 4 };

    const expected = ['Object{', '  a: "A",', '  b: Object{', '    ba: "BA",', '    bb: "BB"', '  },', '  c: 4', '}'].join('\n');
    assert.equal(stringify(input, { indent: '  ' }), expected);
  });
  it('nested empty object', () => {
    const input = { a: 'A', b: {}, c: 4 };

    const expected = ['Object{', '  a: "A",', '  b: Object{},', '  c: 4', '}'].join('\n');
    assert.equal(stringify(input, { indent: '  ' }), expected);
  });
...
describe('circular references', () => {
  it('circular object', () => {
    ...
    const expected = ['Object{', '  circularRef: #circular#,', '  list: [', '    #circular#,', '    #circular#', '  ]', '}'].join('\n');
    assert.equal(stringify(circularObj, { indent: '  ' }), expected);
  });
  it('circular array', () => {
    const circularArray: unknown[] = [3, 5];
    circularArray.push(circularArray);
    const expected = ['[', '  3,', '  5,', '  #circular#', ']'].join('\n');
